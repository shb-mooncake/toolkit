...
//数字千分位分隔
export const getThousandSeparator = (n) => {
    let num = n.toString();
    let len = num.length;
    if (len <= 3) {
        return num;
...
//字符串首字母大写
export const getStrFistLetterUpper = (str) => {
    return str.charAt(0).toUpperCase() + str.slice(1);
};
//手机号中间四位变成*
export const getStrTelFormat = (tel) => {
    tel = String(tel);
    return tel.substr(0, 3) + "****" + tel.substr(7);
};
//驼峰命名转换成短横线命名
export const getStrKebabCase = (str) => {
    return str.replace(/[A-Z]/g, (item) => "-" + item.toLowerCase());
};
//短横线命名转换成驼峰命名
export const getStrCamelCase = (str) => {
    return str.replace(/-([a-z])/g, (i, item) => item.toUpperCase());
};
//全角转换为半角
export const getStrToCDB = (str) => {
    let result = "";
 ...
 */
//数字转化为大写金额
export const getDigitUppercase = (n) => {
    const fraction = ["角", "分"];
...
//数字转化为中文数字
export const getIntToChinese = (value) => {
...
//根据id等属性过滤数组对象
export const filterObjectArr = (arr, property) => {
    let map = {};
    arr.forEach((item) => {
